...
describe('User model', function () {
    ...
    // Do we need a sync force true after?
    ...
    beforeEach(function (done) {
        return User.create({
          email: 'fakeuser1@gha.com',
          password: 'fakeuser1_password'
      })
        .then(function (u) {
          user = u;
          done();
    ...
    it("User should have the correct fields", function (done) {
        expect(user.dataValues).to.include.all.keys(
            ['id',
            'email',
            'password',
            'salt',
    ...
    it("User should require an email and password", function (done) {
        User.create()
